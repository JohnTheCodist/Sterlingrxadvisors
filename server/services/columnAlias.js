/**
 * Per-column mapping memory for one organization.
 *
...
/**
 * Every alias this organization has recorded, as a Map keyed by aliasKey.
 * One query — callers look up many headers against it.
 */
async function loadAliases(organizationId) {
  assertOrgId(organizationId);
  const db = getSql();
  const rows = await db`
    select normalized_header, category, times_confirmed, times_overridden
    from column_alias where organization_id = ${organizationId}
  `;
  const map = new Map();
  for (const r of rows) {
...
/**
 * Resolve raw headers against this organization's memory.
 *
 * @returns {Record<string, string>} rawHeader -> category, ready to be handed
 *          to the normalizer as userMapping.
 */
async function resolveKnownColumns(organizationId, rawHeaders) {
  if (!organizationId || !rawHeaders || rawHeaders.length === 0) return {};
  const aliases = await loadAliases(organizationId);
 ...
 */
async function recordAlias(organizationId, rawHeader, category, { overridden = false, source = 'user' } = {}) {
  assertOrgId(organizationId);
  ...
  await db`
    insert into column_alias
      (organization_id, normalized_header, category, times_confirmed, times_overridden, source, last_seen)
    values (
      ${organizationId}, ${key}, ${category},
      ${overridden ? 0 : 1}, ${overridden ? 1 : 0}, ${source}, now()
    )
    on conflict (organization_id, normalized_header) do update set
 ...
 */
async function recordMapping(organizationId, mapping, overrides = {}, source = 'web') {
  for (const [rawHeader, category] of Object.entries(mapping || {})) {
    if (!category) continue;
    await recordAlias(organizationId, rawHeader, category, {
